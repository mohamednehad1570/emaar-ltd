...
import { solutionsIndexData } from '@/lib/data/solutions'

// Client shell for /solutions — owns animation and language selection
export default function SolutionsPageClient() {
  const { language, isRTL } = useLanguage()
  const t = solutionsIndexData[language]
      ...
      <section className="pt-[52px] pb-20 bg-off-white">
        <Container className="pt-16">
          ...
          <motion.h1
            ...
            className="font-extrabold text-ink-heading mb-4"
          ...
          <motion.p
            ...
            className="text-ink-body text-lg max-w-xl mb-16"
          >
            {t.subheading}
          </motion.p>
          ...
          >
            {t.cards.map((card) => (
              <motion.div key={card.href} variants={fadeUp}>
                <Link
                  href={card.href}
                  className="group flex flex-col justify-between h-full bg-surface-white border border-border-light p-8 hover:border-brand-red transition-colors duration-300 min-h-[220px]"
                >
                  <div>
                    <h2 className="text-2xl font-bold text-ink-heading mb-3">{card.title}</h2>
                    <p className="text-ink-body text-base">{card.description}</p>
                  </div>
      ...
      <section className="py-20 bg-brand-dark text-white">
        <div className={`max-w-3xl mx-auto px-6 text-center ${isRTL ? 'rtl' : ''}`}>
          ...
          <a
            ...
            className={`inline-flex items-center gap-2 px-8 py-4 bg-brand-red hover:bg-brand-red-deep text-white font-bold text-base transition-colors ${isRTL ? 'flex-row-reverse' : ''}`}
          >
            {t.ctaButton}
