    ...
    <div className={`min-h-screen bg-brand-bg ${isRTL ? 'rtl' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <section className="relative h-[80vh] flex items-center justify-center overflow-hidden bg-brand-dark">
        <div className="absolute inset-0 z-0 opacity-40">
          <div className="w-full h-full bg-gradient-to-br from-brand-dark to-[#333]" />
        </div>
        <div className="relative z-10 max-w-7xl mx-auto px-6 text-center text-white">
          <motion.span initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="inline-block py-1 px-4 rounded-full bg-brand-silver/20 border border-brand-silver/50 text-brand-silver text-sm font-semibold mb-6">
            {t.hero.subtitle}
          </motion.span>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="flex gap-4 justify-center">
            <Link href="/contact?type=commercial" className="inline-flex items-center gap-2 px-8 py-4 bg-brand-red text-white rounded-full font-bold text-lg hover:bg-brand-red-dark transition-all shadow-lg">
              {t.hero.cta}
            </Link>
          </motion.div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid md:grid-cols-3 gap-8">
            {t.capabilities.map((cap, i) => {
              ...
              return (
                <motion.div key={i} variants={fadeUp} initial="hidden" whileInView="visible" viewport={viewportOnce} transition={{ delay: i * 0.1 }} className="bg-brand-bg border border-brand-silver/10 p-8 rounded-2xl">
                  <div className="mb-6 p-4 bg-white rounded-xl inline-block shadow-sm"><Icon className="w-8 h-8 text-brand-red" /></div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-12">
            <div className="md:w-1/2">
              ...
              <div className="flex flex-wrap gap-4">
                <div className="flex items-center gap-2 bg-white/10 px-4 py-2 rounded-lg"><FileText className="w-4 h-4 text-brand-red" /><span>PDF Specifications</span></div>
                <div className="flex items-center gap-2 bg-white/10 px-4 py-2 rounded-lg"><Cog className="w-4 h-4 text-brand-red" /><span>CAD Drawings</span></div>
              </div>
            </div>
            <div className="md:w-auto">
              <Link href="/tech" className="inline-flex items-center gap-2 px-8 py-4 bg-white text-brand-dark rounded-xl font-bold text-lg hover:bg-brand-bg transition-all">
